...
# Custom Claude Code statusline - Created: 2025-01-07T09:00:00.000Z
# Theme: detailed | Colors: true | Icons: nerd-font | Features: directory, git, model, context, usage, session, tokens, burnrate
...
if [ "$HAS_JQ" -eq 1 ]; then
  ...
  model_version=""
  session_id=$(echo "$input" | grep -o '"session_id"[[:space:]]*:[[:space:]]*"[^"]*"' | sed 's/.*:[[:space:]]*"\([^"]*\)".*/\1/')
...
# ---- render statusline ----
# Line 1: Core info (directory, git, model, output style, claude code version)
printf '%s󰉋%s %s%s%s' "$(dir_color)" "$(rst)" "$(dir_color)" "$current_dir" "$(rst)"
if [ -n "$git_branch" ]; then
  printf '  %s󰊢%s %s%s%s' "$(git_color)" "$(rst)" "$(git_color)" "$git_branch" "$(rst)"
fi
printf '  %s󱤇%s %s%s%s' "$(model_color)" "$(rst)" "$(model_color)" "$model_name" "$(rst)"
if [ -n "$model_version" ] && [ "$model_version" != "null" ]; then
  printf '  %s󰈙%s %s%s%s' "$(version_color)" "$(rst)" "$(version_color)" "$model_version" "$(rst)"
fi
if [ -n "$output_style" ] && [ "$output_style" != "null" ]; then
  printf '  %s󰏘%s %s%s%s' "$(style_color)" "$(rst)" "$(style_color)" "$output_style" "$(rst)"
fi
if [ -n "$cc_version" ] && [ "$cc_version" != "null" ]; then
  printf '  %s󰘚%s %sv%s%s' "$(cc_version_color)" "$(rst)" "$(cc_version_color)" "$cc_version" "$(rst)"
fi
...
if [ -n "$context_pct" ]; then
  ...
  line2="$(context_color)󰾶$(rst) $(context_color)Context: [${context_bar}] ${context_remaining_pct}% remaining$(rst)"
fi
if [ -n "$session_txt" ]; then
  if [ -n "$line2" ]; then
    line2="$line2  $(session_color)󱎫$(rst) $(session_color)${session_txt}$(rst) $(session_color)[${session_bar}]$(rst)"
  else
    line2="$(session_color)󱎫$(rst) $(session_color)${session_txt}$(rst) $(session_color)[${session_bar}]$(rst)"
  fi
fi
if [ -z "$line2" ] && [ -z "$context_pct" ]; then
  line2="$(context_color)󰾶$(rst) $(context_color)Context: [░░░░░░░░░░] TBD$(rst)"
fi
...
if [ -n "$cost_usd" ] && [[ "$cost_usd" =~ ^[0-9.]+$ ]]; then
  if [ -n "$cost_per_hour" ] && [[ "$cost_per_hour" =~ ^[0-9.]+$ ]]; then
    ...
    line3="$(cost_color)󰈙$(rst) $(cost_color)\$$(printf '%.2f' "$cost_usd")$(rst) ($(burn_color)\$${cost_per_hour_formatted}/h$(rst))"
  else
    line3="$(cost_color)󰈙$(rst) $(cost_color)\$$(printf '%.2f' "$cost_usd")$(rst)"
  fi
fi
if [ -n "$tot_tokens" ] && [[ "$tot_tokens" =~ ^[0-9]+$ ]]; then
  if [ -n "$tpm" ] && [[ "$tpm" =~ ^[0-9.]+$ ]]; then
    ...
    if [ -n "$line3" ]; then
      line3="$line3  $(usage_color)󱪙$(rst) $(usage_color)${tot_tokens} tok (${tpm_formatted} tpm)$(rst)"
    else
      line3="$(usage_color)󱪙$(rst) $(usage_color)${tot_tokens} tok (${tpm_formatted} tpm)$(rst)"
    fi
  else
    if [ -n "$line3" ]; then
      line3="$line3  $(usage_color)󱪙$(rst) $(usage_color)${tot_tokens} tok$(rst)"
    else
      line3="$(usage_color)󱪙$(rst) $(usage_color)${tot_tokens} tok$(rst)"
    fi
  fi
fi
