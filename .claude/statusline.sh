...
# Custom Claude Code statusline - Created: 2025-09-30T03:31:10.667Z
...
if [ -z "$line2" ] && [ -z "$context_pct" ]; then
  line2="$(context_color)󰾶$(rst) $(context_color)Context: [░░░░░░░░░░] 100% remaining$(rst)"
fi
...
if [ -n "$tot_tokens" ] && [[ "$tot_tokens" =~ ^[0-9]+$ ]]; then
  if [ -n "$tpm" ] && [[ "$tpm" =~ ^[0-9.]+$ ]]; then
    ...
    if [ -n "$line3" ]; then
      line3="$line3  $(usage_color)󱪙$(rst) $(usage_color)${formatted_tokens} tokens (${formatted_tpm} tpm)$(rst)"
    else
      line3="$(usage_color)󱪙$(rst) $(usage_color)${formatted_tokens} tokens (${formatted_tpm} tpm)$(rst)"
    fi
  else
    formatted_tokens=$(format_number "${tot_tokens}")
    if [ -n "$line3" ]; then
      line3="$line3  $(usage_color)󱪙$(rst) $(usage_color)${formatted_tokens} tokens$(rst)"
    else
      line3="$(usage_color)󱪙$(rst) $(usage_color)${formatted_tokens} tokens$(rst)"
    fi
  fi
fi
